...
def findAndValidateApps(dir: str):
    ...
    for subdir in os.scandir(dir):
        ...
        if os.path.isfile(os.path.join(app_dir, "app.yml.jinja")):
            ...
            os.system("docker run --rm -v {}:/apps -u 1000:1000 {} /app-cli preprocess --app-name '{}' --port-map /apps/ports.json /apps/{}/app.yml.jinja /apps/{}/app.yml --services 'c-lightning'".format(dir, dependencies['app-cli'], subdir.name, subdir.name, subdir.name))
